...
def main():
    startTime = time.time()
    pool = threadpool.ThreadPool(10)  # 创建线程池，线程数为10个
    global parseQue
    ...
    downloadQue = []
    for i in range(1,12311):#12311
        params = {"pageNo": i, "totalPageCount": 12310}
        downloadQue.append(params)

    """
        将请求参数放到队列中,当队列不为空时循环创建线程执行任务
    """
    downloadRequests = threadpool.makeRequests(sendReq,downloadQue)

    [pool.putRequest(req) for req in downloadRequests]
    pool.wait()
    ...
    #     paramList.append(parseQue.get())
    #
    # parseRequests = threadpool.makeRequests(parseDoc, paramList)  #创建任务
    # [pool.putRequest(req) for req in parseRequests]
    # pool.wait()
    endTime = time.time()
